...
# Switchs('False' for actual data processing)
...
for i, idx in enumerate(data[1:]):
  ni = i+1
  if float(data[ni][2]) != -500 and float(data[ni][13]) > 0 and float(data[ni][2]) > 0:# and float(data[i][2]) < 1. and np.log10(float(data[i][5])) < 5.7:
    M_init_list.append(float(data[ni][0]))
    H_env_fin_list.append(float(data[ni][2]))
    H_env_fin_log_list.append(np.log10(float(data[ni][2])))
    H_env_round_list.append(round(5*np.log10(float(data[ni][2]))))
    M_fin_list.append(float(data[ni][3]))
    M_fin_log_list.append(np.log10(float(data[ni][3])))
    R_fin_list.append(np.log10(float(data[ni][4])))
    L_fin_list.append(np.log10(float(data[ni][5])))
    Teff_list.append(np.log10(float(data[ni][6])))
    g_list.append(np.log10(float(data[ni][7])))
    sL_fin_list.append(np.log10(float(data[ni][8])))
    C_core_list.append(np.log10(float(data[ni][10])))
# Grid setting
...
if sandbox_switch is False:
  ...
  tree1.fit(XY1,Z1)
  mlp1 = MLPClassifier(hidden_layer_sizes = [777, 777, 77], activation = 'relu', solver = 'lbfgs', learning_rate = 'adaptive', max_iter = 1000, alpha = 0.005)
  mlp1.fit(XY1,Z1)
...
# Select decision boundary plotting method: 2
  if sL_switch is True:
    prc2 = Perceptron(tol = 1e-3, penalty = 'elasticnet')
    prc2.fit(XY2,Z2)
    tree2 = DecisionTreeClassifier(splitter = 'best', criterion = 'entropy')
    tree2.fit(XY2,Z2)
    mlp2 = MLPClassifier(hidden_layer_sizes = [777, 777, 77], activation = 'relu', solver = 'lbfgs', learning_rate = 'adaptive', max_iter = 1000, alpha = 0.005)
    mlp2.fit(XY2,Z2)
